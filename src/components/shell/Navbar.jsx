...
export default function Navbar(){
  ...
  useEffect(()=>{const onScroll=()=>{const next=window.scrollY>48;if(next&&!scrolledRef.current)setReminderIndex(i=>(i+1)%REMINDERS.length);scrolledRef.current=next;setScrolled(next);};onScroll();window.addEventListener('scroll',onScroll,{passive:true});return()=>window.removeEventListener('scroll',onScroll);},[]);
  ...
  useEffect(()=>{setMenuOpen(false);setMegaOpen(null);setMobileGroup(null);setSettingsOpen(false);},[pathname]);
  ...
  const openDonation=()=>{setMenuOpen(false);setMegaOpen(null);setSettingsOpen(false);setDonationOpen(true);};

  const SettingsPanel=()=> <div className="jic-settings-panel jic-glass" role="dialog" aria-label="Display settings">
    <div className="jic-settings-head"><div><strong>Settings</strong><span>Display preferences</span></div><button onClick={()=>setSettingsOpen(false)} aria-label="Close settings"><X size={17}/></button></div>
    <button className="jic-setting-row" onClick={()=>setTheme(theme==='dark'?'light':'dark')}><span className="jic-setting-icon">{theme==='dark'?<Moon size={17}/>:<Sun size={17}/>}</span><span><strong>Appearance</strong><small>{theme==='dark'?'Dark':'Light'} mode</small></span><b>{theme==='dark'?'Dark':'Light'}</b></button>
    <button className="jic-setting-row" onClick={()=>setGlass(v=>!v)}><span className="jic-setting-icon"><span className={cn('glass-switch',glass&&'on')}><i/></span></span><span><strong>Glass effects</strong><small>Translucent interface surfaces</small></span><b>{glass?'On':'Off'}</b></button>
  </div>;
    ...
    <header className={cn('jic-header fixed inset-x-0 top-0 z-50',scrolled&&'is-scrolled')}>
      <div className="mx-auto max-w-[1500px] px-2 sm:px-4 pt-2">
        <div className="jic-glass jic-info-shell overflow-hidden">
          ...
          <div className="jic-utility-row"><Link to="/prayer-times/jummah" className="jic-jummah-mini" aria-label="Friday Jummah times"><span>JUMMAH</span><strong>{jummahTimes.map(t=>shortTime(t.prayer)).join(' · ') || '—'}</strong></Link><button className={cn('jic-radio',playing&&'is-live')} onClick={toggleRadio}>{playing?<Pause size={15}/>:<Play size={15}/>}<span>JIC Radio</span><i className="live-dot"/><small>{radioError?'Retry':'Live'}</small></button></div>
        </div>
        ...
        <div className="jic-mainnav mt-2">
          ...
          <div className="jic-nav-actions"><Link to="/admin/login" className="header-icon jic-admin-entry" aria-label="Admin login" title="Admin"><LogIn size={18}/></Link><div className="jic-settings-wrap"><button className="header-icon" onClick={()=>{setSettingsOpen(v=>!v);setMenuOpen(false)}} aria-label="Settings" title="Settings"><Settings2 size={18}/></button><AnimatePresence>{settingsOpen&&<motion.div initial={{opacity:0,y:-6}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-6}}><SettingsPanel/></motion.div>}</AnimatePresence></div><button type="button" onClick={openDonation} className="donate-button" aria-label="Donate to Jamatia Islamic Centre"><Heart size={18}/><span>Donate</span></button><button className="header-icon" onClick={()=>{setMenuOpen(v=>!v);setSettingsOpen(false)}} aria-label="Menu">{menuOpen?<X size={23}/>:<Menu size={23}/>}</button></div>
        </div>
