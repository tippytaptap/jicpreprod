  ...
  return <header className={cn('jic-header fixed inset-x-0 top-0 z-50',scrolled&&'is-scrolled')}>
    <div className="mx-auto max-w-[1500px] px-2 sm:px-4 pt-2">
      ...
      <div className="jic-mainnav mt-2">
        ...
        <nav className="jic-desktop-nav">{NAV_GROUPS.map(({name,path,children})=><NavLink key={path} to={path} end={path==='/'} className={({isActive})=>cn('nav-pill',isActive&&'active')}>{name}{children.length>0&&<ChevronDown size={13}/>}</NavLink>)}</nav>
      ...
      {activeGroup?.children?.length>0&&<nav className="jic-subnav" aria-label={`${activeGroup.name} sections`}>{activeGroup.children.map(item=><Link key={item.path} to={item.path} className="jic-subnav-link">{item.name}</Link>)}</nav>}
      ...
      <AnimatePresence>{menuOpen&&<motion.div initial={{opacity:0,y:-8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} className="jic-mobile-menu jic-glass">
        ...
        <div className="jic-menu-scroll">{NAV_GROUPS.map(({name,path,children})=><div className="jic-menu-group" key={path}><NavLink to={path} end={path==='/'} onClick={()=>setMenuOpen(false)} className={({isActive})=>cn('jic-menu-link',isActive&&'active')}>{name}<span>›</span></NavLink>{children.length>0&&<div className="jic-menu-children">{children.filter(child=>child.path!==path).map(child=><Link key={child.path} to={child.path} onClick={()=>setMenuOpen(false)}>{child.name}</Link>)}</div>}</div>)}<Link to="/admin" onClick={()=>setMenuOpen(false)} className="jic-menu-admin"><LogIn size={17}/> Admin login</Link><Link to="/projects#donate" onClick={()=>setMenuOpen(false)} className="jic-menu-donate"><Heart size={18}/> Donate</Link></div>
