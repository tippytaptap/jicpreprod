  ...
  return <main className="admin-login-page">
    ...
    <section className="admin-login-card jic-popup-surface" aria-labelledby="admin-login-title">
      ...
      <form onSubmit={submit} className="admin-login-form">
        <label><span>Email address</span><input type="email" required inputMode="email" autoCapitalize="none" autoComplete="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="admin@example.org"/></label>
        <label><span>Password</span><div className="admin-login-password"><LockKeyhole size={17}/><input type="password" required autoComplete="current-password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="Your password"/></div></label>
        <button disabled={busy||loading}><LogIn size={18}/>{busy?'Signing in…':'Sign in securely'}</button>
      </form>
      <p className="admin-login-footnote">Only approved JIC staff accounts can access the dashboard.</p>
    </section>
  </main>;
}
